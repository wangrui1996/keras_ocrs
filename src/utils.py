...
class DataGenerator(keras.utils.Sequence):
    ...
    def set_dataset(self, image_path, labels_path, train=False):
        self.image_path = image_path
        res = []
        with open(labels_path, 'r') as f:
            lines = f.readlines()
    ...
    def __len__(self):
        ...
        return int(numpy.floor(self.lines.shape[0] / self.batch_size))
    ...
    def __getitem__(self, index):
        ...
        for idx, line in enumerate(shufimagefile):
            ...
            label = text_to_labels(text)
            labels[idx, 0:len(label)] = label
            input_length[idx] = self.img_w // self.downsample_factor
            label_length[idx] = len(text)
        inputs = {'the_input': X_data,
                  'the_labels': labels,
                  'input_length': input_length,
